...
public class GameServer {
    ...
    private void onSetGameCharacter(Message message, Session userSession) {
        ...
        try {
            ...
            if (user.getPlayingGame().getCharacterMaster().equals(game.getCharacterMaster())) {
                character=game.getCharacterMaster();
            }
            else character=game.getCharacterGuest();

            switch (characterType) {
                case GameCharacter.KNIGHT:
                    character = new KnightCharacter();
                    break;
                case GameCharacter.MEDUSA:
                    character = new MedusaCharacter();
                    break;
                case GameCharacter.HOT_GIRL:
                    character = new MedusaCharacter();
                    break;
                case GameCharacter.DRACULA:
                    character = new DraculaCharacter();
                    break;
                default:
            ...
            content.add("character", gson.toJsonTree(character));
            response.setContent(content);
            response.setStatus(200);

            rivalMessageContent.addProperty("character_type", characterType);
            rivalMessageContent.add("character", gson.toJsonTree(character));
            rivalMessage.setContent(content);
            rivalMessage.setStatus(200);

            if (game.checkMaster(character)) {
                game.setMaster(character);
